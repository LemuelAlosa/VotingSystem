...
if (isset($_GET['id'])) {
    $superadminId = $_GET['id'];
    
    $sql_select = "SELECT * FROM `super_admin` WHERE superadmin_Id='$superadminId'";
    $result_select = $conn->query($sql_select);
    if ($result_select->num_rows > 0) {
        $row_select = $result_select->fetch_assoc();
        $username_superadmin = $row_select["username_superadmin"];
        ...
        if ($result_insertReports->num_rows > 0) {
            ...
            if ($result_Superadminreports->num_rows > 0) {
                ...
                $sql_reports = "INSERT INTO `$audittrial_reportbatch`(`reports`, `account`, `date/time`) VALUES ('Superadmin that has a username $username_superadmin has been removed','$superadmin','$currentDateFormatted/$currentTime12Hour')";
                mysqli_query($conn, $sql_reports);
            }
        }
    }

    $sql = "DELETE FROM `super_admin` WHERE superadmin_Id ='$superadminId'";
