...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST['superadminusername']) && $_POST['superadminpassword'] != "") {

        $superadminusername = strtolower($_POST["superadminusername"]);
        $superadminpassword = $_POST["superadminpassword"];

        $finalsuperadminpassword = hash('sha256', $Salt . $superadminpassword . $Pepper);

        $sql = "INSERT INTO super_admin (username_superadmin, password_superadmin, status) values ('$superadminusername','$finalsuperadminpassword','deactivated')";

        echo "<script>alert($superadminusername);</script>";
        ...
        if ($conn->query($sql) === TRUE){
            ...
            if ($result_insertReports->num_rows > 0) {
                ...
                if ($result_Superadminreports->num_rows > 0) {
                    ...
                    $sql_reports = "INSERT INTO `$audittrial_reportbatch`(`reports`, `account`, `date/time`) VALUES ('Added new superadmin named $superadminusername','$superadmin','$currentDateFormatted/$currentTime12Hour')";
