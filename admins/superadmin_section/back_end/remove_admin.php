...
if (isset($_GET['id'])) {
    ...
    $sql_select = "SELECT * FROM `admins` WHERE admin_id='$adminId'";
    $result_select = $conn->query($sql_select);
    if ($result_select->num_rows > 0) {
        $row_select = $result_select->fetch_assoc();
        $username_admin = $row_select["username_admin"];
        ...
        if ($result_insertReports->num_rows > 0) {
            ...
            if ($result_Superadminreports->num_rows > 0) {
                ...
                $sql_reports = "INSERT INTO `$audittrial_reportbatch`(`reports`, `account`, `date/time`) VALUES ('Admin that has a username $username_admin has been removed','$superadmin','$currentDateFormatted/$currentTime12Hour')";
