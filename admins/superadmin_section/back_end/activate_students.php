...
if(isset($_SESSION['Batch'])){
    ...
    $sqlUpdateStudents = "UPDATE `students_batch$currentBatch` SET account = 'activated'";
}else{
    $defaultBatch = $_SESSION['defaultBatch'];
    $sqlUpdateStudents = "UPDATE `students_batch$defaultBatch` SET account = 'activated'";
}
...
if ($resultUpdateStudents === TRUE) {
    ...
    if ($result_insertReports->num_rows > 0) {
        ...
        if ($result_Superadminreports->num_rows > 0) {
            ...
            $sql_reports = "INSERT INTO `$audittrial_reportbatch`(`reports`, `account`, `date/time`) VALUES ('Activated all the students.','$superadmin','$currentDateFormatted/$currentTime12Hour')";
